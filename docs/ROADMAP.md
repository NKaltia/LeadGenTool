...
## Process Practices (apply from Milestone 2.5 onward)

- **Branching**: work on a dedicated branch per milestone or feature
  (`feature/<short-name>`), never commit directly to `main`. Open a Pull
  Request to merge, even solo — it is the review checkpoint and the place CI
  results are visible before merge.
- **CI**: every push and PR runs typecheck, lint, test, and build
  automatically. A milestone is not "done" if it only passes these checks
  locally — it must pass in CI on the PR.
- **ADRs**: when a milestone introduces a new dependency, or a structural
  decision with real alternatives (not a trivial choice), write a short ADR
  in `docs/decisions/` at the time the decision is made — not reconstructed
  from memory later. See `docs/decisions/0000-template.md`.
...
# Milestone 2.5 — Engineering Process Upgrade

## Goal

Adopt the git branching, CI, and decision-recording practices that will apply
for the rest of the project, before scope grows further. This milestone
produces no user-facing feature — it upgrades how the work itself is done.

## Scope

- Learn and practice the branch → commit → push → Pull Request → merge
  workflow, applied to the very next small change.
- Add a GitHub Actions CI workflow (`.github/workflows/ci.yml`) that runs on
  every push and PR: install, typecheck, lint, test, build. The test job
  must run against a real PostgreSQL service container, not a mock.
- Confirm `docs/decisions/` ADRs already recorded (0001–0005) are accurate;
  add one for the website-audit safety design once Milestone 2's audit logic
  is finalized (see the SSRF-protection gap noted during review).
- Extract the ad hoc per-route error responses from Milestone 1 into a single
  centralized Express error-handling middleware, so "consistent error
  responses" is enforced in one place instead of repeated per route.

## Do Not Add Yet

- Deployment / CD to a real environment (that stays in Milestone 9)
- Authentication
- Redis / BullMQ

## Acceptance Criteria

- At least one full branch → PR → CI-green → merge cycle has been completed.
- CI fails the PR if a test, lint, or typecheck error is introduced (verify
  this once deliberately, with a throwaway broken change, then revert it).
- All Express routes share one error-handling middleware.
- `docs/decisions/` reflects every significant decision made so far.

## What I Should Understand

- git branches, remotes, and the difference between local and remote branches
- what a Pull Request actually is (a diff + discussion, not a git concept)
- why CI runs in a clean environment instead of trusting local runs
- `npm ci` vs `npm install`
- Express error-handling middleware signature `(err, req, res, next)` and why
  it must be registered last
...
- Extend the CI pipeline from Milestone 2.5 with deployment (CD): build and
  ship the backend, database, worker, and frontend as appropriate.
- Add health checks and deployment documentation.
- Confirm `docs/decisions/` covers every major choice made through this
  milestone (CI itself was already recorded as an ADR in Milestone 2.5).

## Acceptance Criteria

- The project is deployable from documented instructions.
- CI (from Milestone 2.5) still runs on every pull request and push.
