...
Significant structural decisions and their alternatives are recorded as
Architecture Decision Records in `docs/decisions/` at the time they are made,
so the reasoning behind the current shape of the system stays available
without having to be reconstructed from memory later.
